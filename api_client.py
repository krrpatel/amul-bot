...
class AmulAPIClient:
    ...
    def set_store_preferences(self, pincode: str) -> bool:
        """Set store location by pincode."""
        input_box = self.wait.until(
            ec.visibility_of_element_located(
                (By.CSS_SELECTOR, 'input[placeholder="Enter Your Pincode"]')
            )
        )
        input_box.clear()

        # Use the provided pincode
        ...
        self.wait.until(
            ec.visibility_of_element_located(
                (By.CSS_SELECTOR, "div.pincode_wrap span.ms-2.fw-semibold")
            )
        )

        logger.info("✅ Pin code confirmed: %s", pincode)
        return True

    # -------------------- PRODUCTS --------------------

    def get_products(self) -> List[Dict[str, Any]]:
        """Fetch all protein products."""
        self.driver.get("https://shop.amul.com/en/browse/protein")
        time.sleep(2)
    ...
    def _get_product_details_with_driver(
        ...
        driver.get(f"https://shop.amul.com/en/product/{alias}")
        time.sleep(0.8)
    ...
    def get_product_details_parallel(
        ...
        def fetch_single_product(alias: str):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(fetch_single_product, a): a for a in aliases}
    ...
    def get_store_from_pincode(self, _: str) -> str:
        """Placeholder method for store lookup."""
        return ""
